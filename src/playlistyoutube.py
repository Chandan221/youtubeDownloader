...
class YouTubeDownloaderApp:
    ...
    def build_header(self):
        ...
        self._label(left, "Download single videos or entire playlists", "secondary",
    ...
    def build_url_row(self, parent):
        ...
        self._label(row, "Video or Playlist URL", "normal",
    ...
    def build_checkbox_row(self, parent):
        ...
        self.playlist_cb = tk.Checkbutton(row,
            text="This is a playlist", variable=self.is_playlist,
            font=font_spec(12),
    ...
    def build_quality_row(self, parent):
        ...
        self.quality_combo = ttk.Combobox(row,
            width=62, state="readonly", font=font_spec(10))
    ...
    def build_download_btn(self, parent):
        ...
        self.download_button = self._btn(row, "\u25BC  Download", "primary",
    ...
    def build_progress_card(self):
        ...
        self.progress = ttk.Progressbar(card, orient="horizontal",
            length=680, mode="determinate", style="TProgressbar")
    ...
    def fetch_qualities(self):
        ...
        try:
            ...
            for s in self.video_streams:
                ...
                options.append(f"{s.resolution} | {s.fps}fps | {round(s.filesize / (1024 * 1024), 2)} MB")
            self.quality_combo["values"] = options
            self.quality_combo.current(0)
            self.set_status("Qualities loaded", True)
    ...
    def start_download(self):
        ...
        if not url:
            messagebox.showerror("Error", "Please enter a YouTube URL.")
            return
        quality = self.get_selected_resolution()
        if not quality:
            messagebox.showerror("Error", "Please select a quality.")
            return
        thread = threading.Thread(target=self.download_task, args=(url, quality), daemon=True)
        thread.start()
    ...
    def download_task(self, url, selected_quality):
        ...
        try:
            ...
            self.download_button.config(state="normal",
                bg=self.colors["primary"], text="\u25BC  Download")

    def download_playlist(self, playlist_url, selected_quality):
        playlist = Playlist(playlist_url)
        title = clean_filename(playlist.title or "YouTube_Playlist")
        folder = os.path.join(self.download_path, title)
        os.makedirs(folder, exist_ok=True)
        total = len(playlist.video_urls)
        self.log(f"Playlist: {title}")
        self.log(f"Total videos: {total}")
        self.log(f"Quality: {selected_quality}")
        for i, vurl in enumerate(playlist.video_urls, 1):
            try:
                self.set_status(f"Downloading {i}/{total}")
                self.log(f"Downloading {i}/{total}: {vurl}")
                yt = YouTube(vurl, on_progress_callback=self.on_progress)
                self.download_video_by_quality(yt, selected_quality, folder, prefix=f"{i:02d}_")
                self.log(f"Done: {yt.title}")
            except Exception as e:
                self.log(f"Failed: {vurl}")
                self.log(f"Reason: {e}")
    ...
    def download_video_by_quality(self, yt, selected_quality, output_folder, prefix=""):
        ...
        if video is None:
            self.log(f"{selected_quality} not available for {yt.title}. Using best available.")
            video = (yt.streams
                .filter(adaptive=True, only_video=True, file_extension="mp4")
        ...
        tv = os.path.join(output_folder, f"{prefix}temp_video.mp4")
        ta = os.path.join(output_folder, f"{prefix}temp_audio.mp4")
        out = os.path.join(output_folder, f"{prefix}{title}_{video.resolution}.mp4")
        self.set_status(f"Downloading video ({video.resolution})...")
        video.download(output_path=output_folder, filename=f"{prefix}temp_video.mp4")
        self.set_status("Downloading audio...")
        audio.download(output_path=output_folder, filename=f"{prefix}temp_audio.mp4")
        self.set_status("Merging with FFmpeg...")
        self.root.update_idletasks()
        ffmpeg_bin = get_ffmpeg_path() or "ffmpeg"
        subprocess.run([ffmpeg_bin, "-y", "-i", tv, "-i", ta,
