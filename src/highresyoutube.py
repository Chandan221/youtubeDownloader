...
class YouTubeDownloaderApp:
    def __init__(self, root):
        self.root = root
        self.root.title("YouTube HD Downloader")
    ...
    def build_header(self):
        ...
        self._label(left, "\U0001F3AC YouTube HD Downloader", "heading",
            font=font_spec(18, "bold")).pack(anchor="w")

        self._label(left, "1080p / 2K / 4K video downloads", "secondary",
    ...
    def _build_url_row(self, parent):
        ...
        self.url_entry = tk.Entry(input_frame,
            ...
            highlightbackground=self.colors["border"],
            highlightcolor=self.colors["border_focus"])
        self.url_entry.pack(side="left", fill="x", expand=True, ipady=9, ipadx=12)
    ...
    def _build_quality_row(self, parent):
        ...
        self.quality_combo = ttk.Combobox(row,
            width=58, state="readonly", font=font_spec(10))
    ...
    def build_progress_card(self):
        ...
        self.progress = ttk.Progressbar(card, orient="horizontal",
            length=600, mode="determinate", style="TProgressbar")
    ...
    def fetch_qualities(self):
        ...
        try:
            ...
            if not self.video_streams:
                messagebox.showerror("Error", "No HD video streams found.")
                return
            options = []
            for s in self.video_streams:
                options.append(f"{s.resolution} | {s.fps}fps | {s.video_codec} | {round(s.filesize / (1024 * 1024), 2)} MB")
    ...
    def start_download(self):
        ...
        if idx == -1:
            ...
            return
        thread = threading.Thread(target=self.download_and_merge, args=(idx,), daemon=True)
        thread.start()

    def download_and_merge(self, selected_index):
        try:
            ...
            video_stream = self.video_streams[selected_index]
            ...
            title = clean_filename(self.yt.title)
            video_file = os.path.join(self.download_path, "temp_video.mp4")
            audio_file = os.path.join(self.download_path, "temp_audio.mp4")
            output_file = os.path.join(self.download_path, f"{title}_{video_stream.resolution}.mp4")

            self.set_status("Downloading video...")
            video_stream.download(output_path=self.download_path, filename="temp_video.mp4")
            self.set_status("Downloading audio...")
            audio_stream.download(output_path=self.download_path, filename="temp_audio.mp4")
            ...
            messagebox.showerror("FFmpeg Error", "FFmpeg is not installed or not added to PATH.")
